...
start:      LD   SP,1000h       ; establish local stack  
            CALL getTime        ; get hour/min/sec/days, etc
            CALL calcDOW        ; get DOW, too!
            LD   HL,stTime 
...
; sec, min, hour, days, day, month, & year
;
getTime:    LD   B,8            ; 8 parameters to retrieve
            LD   HL,sec         ; point to first param
jGT:        LD   A,8            
            SUB  B              ; A = 0 through 7
            OUT  (CLKCMD),A     ; send the RTC command
            IN   A,(CLKDAT)     ; and get the RTC result
            LD   (HL),A         ; store the result
            INC  HL             ; advance pointer
            DEC  B              ; for 8080, dont use DJNZ
            JP   NZ,jGT         ; loop until done            
            RET 
...
pDate:      LD   A,(mon)
            ...
            LD   A,'/'
            CALL putCh 
            LD   A,(year)       ; has y2K error, fixed in next line
...
; RTC variables follow
; they are loaded as a 8-byte block, so keep them together in same order
sec:        .db 00              ; stored as BCD
min:        .db 00              ; stored as BCD
hour:       .db 00              ; stored as BCD
days:       .dw 0000            ; stored as 16-bit word, days since 1/1/78
day:        .db 00              ; stored as BCD
mon:        .db 00              ; stored as BCD, Jan=0, Feb=1, etc
year:       .dw 00              ; stored as pseudo-BCD with Y2K error

; addition variables follow
dow:        .db 00
